...
{
    char name[] = "/tmp/aether-recorder-shared-XXXXXX";
    int fd = mkstemp(name);
    if (fd < 0) {
        return -1;
    }
...
static void help()
{
    printf(R"(Usage: aether-recorder [OPTION]... -f [FILE]...
...
Use Ctrl+C to stop.)");
  ...
  --no-dmabuf               By default, aether-recorder will try to use only GPU buffers and copies if
  ...
  -D, --no-damage           By default, aether-recorder will request a new frame from the compositor
                            ...
                            on, aether-recorder does not use this optimization and continuously
  ...
  -v, --version             Prints the version of aether-recorder.
...
Examples:)");
  ...
  - aether-recorder                         Records the video. Use Ctrl+C to stop recording.
                                        The video file will be stored as recording.mp4 in the
                                        current working directory.

  - aether-recorder -f <filename>.ext       Records the video. Use Ctrl+C to stop recording.
  ...
  - aether-recorder -a                      Records the video and audio. Use Ctrl+C to stop recording.
                                        The video file will be stored as recording.mp4 in the
                                        current working directory.

  - aether-recorder -a -f <filename>.ext    Records the video and audio. Use Ctrl+C to stop recording.
                                        The video file will be stored as <filename>.ext in the
                                        current working directory.)");
#endif
    printf(R"(
...
static void apply_config_defaults(
    const RecorderConfig& config,
    FrameWriterParams& params,
    std::string& cmdline_output,
    bool& force_no_dmabuf,
    bool& force_overwrite)
{
    if (config.file && !config.file->empty())
    {
        params.file = *config.file;
    }

    if (config.auto_filename.value_or(false))
    {
        params.file = generate_recording_filename(config, params.file);
    }

    if (config.output && !config.output->empty())
    {
        cmdline_output = *config.output;
    }

    if (config.geometry && !config.geometry->empty())
    {
        selected_region.set_from_string(*config.geometry);
    }

    if (config.overwrite.value_or(false))
    {
        force_overwrite = true;
    }

    if (config.log.value_or(false))
    {
    ...
    if (config.muxer && !config.muxer->empty())
    {
        params.muxer = *config.muxer;
    }

    if (config.pixel_format)
    {
        params.pix_fmt = *config.pixel_format;
    }

    if (config.filter && !config.filter->empty())
    {
        params.video_filter = *config.filter;
    }
    ...
    if (config.bframes)
    {
        params.bframes = *config.bframes;
    }

    if (config.buffrate && *config.buffrate > 0)
    {
        params.buffrate = *config.buffrate;
    }

    if (config.device && !config.device->empty())
    {
        params.hw_device = *config.device;
    }

    if (config.no_dmabuf.value_or(false))
    {
        force_no_dmabuf = true;
    }

    if (config.no_damage.value_or(false))
    {
        use_damage = false;
    }

    for (const auto& param : config.video_params)
    {
        parse_codec_opts(params.codec_options, param);
    }

#ifdef HAVE_AUDIO
    if (config.audio_enabled.value_or(false))
    {
        params.enable_audio = true;
        if (config.audio_device && !config.audio_device->empty())
        {
            audioParams.audio_source = strdup(config.audio_device->c_str());
        }
    }
    ...
    {
        audioParams.audio_backend = config.audio_backend->c_str();
    }
#endif

    if (config.audio_codec && !config.audio_codec->empty())
    {
        params.audio_codec = *config.audio_codec;
    }

    if (config.sample_rate && *config.sample_rate > 0)
    {
        params.sample_rate = *config.sample_rate;
    }

    if (config.sample_format && !config.sample_format->empty())
    {
        params.sample_fmt = *config.sample_format;
    }

    for (const auto& param : config.audio_params)
    {
        parse_codec_opts(params.audio_codec_options, param);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr << "Could not load recorder config: " << config_error << std::endl;
    }
    if (!config.auto_filename)
    {
        config.auto_filename = true;
    }
    apply_config_defaults(config, params, cmdline_output, force_no_dmabuf, force_overwrite);
