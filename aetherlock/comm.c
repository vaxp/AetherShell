...
static ssize_t read_full(int fd, void *dst, size_t size) {
	...
	while (offset < size) {
		...
		if (n < 0) {
			...
			aetherlock_log_errno(LOG_ERROR, "read() failed");
			return -1;
		} else if (n == 0) {
			if (offset == 0) {
				return 0;
			}
			aetherlock_log(LOG_ERROR, "read() failed: unexpected EOF");
			return -1;
		}
		offset += n;
	}
...
static bool write_full(int fd, const void *src, size_t size) {
	...
	while (offset < size) {
		...
		if (n <= 0) {
			...
			aetherlock_log_errno(LOG_ERROR, "write() failed");
			return false;
		}
		offset += n;
	}
...
ssize_t read_comm_request(char **buf_ptr) {
	...
	aetherlock_log(LOG_DEBUG, "received pw check request");
	...
	if (read_full(fd, buf, size) <= 0) {
		aetherlock_log_errno(LOG_ERROR, "failed to read pw");
		return -1;
	}
...
bool spawn_comm_child(void) {
	if (pipe(comm[0]) != 0) {
		aetherlock_log_errno(LOG_ERROR, "failed to create pipe");
		return false;
	}
	if (pipe(comm[1]) != 0) {
		aetherlock_log_errno(LOG_ERROR, "failed to create pipe");
	...
	pid_t child = fork();
	if (child < 0) {
		aetherlock_log_errno(LOG_ERROR, "failed to fork");
		return false;
	} else if (child == 0) {
		struct sigaction sa = {
			.sa_handler = SIG_IGN,
...
bool write_comm_request(struct aetherlock_password *pw) {
	bool result = false;
	int fd = comm[0][1];

	size_t size = pw->len + 1;
	if (!write_full(fd, &size, sizeof(size))) {
		aetherlock_log_errno(LOG_ERROR, "Failed to write pw size");
		goto out;
	}

	if (!write_full(fd, pw->buffer, size)) {
		aetherlock_log_errno(LOG_ERROR, "Failed to write pw buffer");
		goto out;
	}
...
bool read_comm_reply(bool *auth_success) {
	if (read_full(comm[1][0], auth_success, sizeof(*auth_success)) <= 0) {
		aetherlock_log(LOG_ERROR, "Failed to read pw result");
		return false;
	}
	return true;
}
