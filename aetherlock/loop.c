...
struct loop *loop_create(void) {
	...
	if (!loop) {
		aetherlock_log(LOG_ERROR, "Unable to allocate memory for loop");
		return NULL;
	}
	loop->fd_capacity = 10;
...
void loop_poll(struct loop *loop) {
	...
	if (ret < 0 && errno != EINTR) {
		aetherlock_log_errno(LOG_ERROR, "poll failed");
		exit(1);
	}
...
void loop_add_fd(struct loop *loop, int fd, short mask,
	...
	if (!event) {
		aetherlock_log(LOG_ERROR, "Unable to allocate memory for event");
		return;
	}
...
struct loop_timer *loop_add_timer(struct loop *loop, int ms,
	...
	if (!timer) {
		aetherlock_log(LOG_ERROR, "Unable to allocate memory for timer");
		return NULL;
	}
