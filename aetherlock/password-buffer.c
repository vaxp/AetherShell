...
static bool password_buffer_lock(char *addr, size_t size) {
	...
	while (mlock(addr, size) != 0 && retries > 0) {
		...
		case EAGAIN:
			retries--;
			if (retries == 0) {
				aetherlock_log(LOG_ERROR, "mlock() supported but failed too often.");
				return false;
			}
			break;
		case EPERM:
			aetherlock_log_errno(LOG_ERROR, "Unable to mlock() password memory: Unsupported!");
			mlock_supported = false;
			return true;
		default:
			aetherlock_log_errno(LOG_ERROR, "Unable to mlock() password memory.");
			return false;
		}
	}
...
static bool password_buffer_unlock(char *addr, size_t size) {
	if (mlock_supported) {
		if (munlock(addr, size) != 0) {
			aetherlock_log_errno(LOG_ERROR, "Unable to munlock() password memory.");
			return false;
		}
	}
...
char *password_buffer_create(size_t size) {
	...
	if (result) {
		...
		errno = result;
		aetherlock_log_errno(LOG_ERROR, "failed to alloc password buffer");
		return NULL;
	}
