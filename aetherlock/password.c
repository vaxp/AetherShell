...
static bool backspace(struct aetherlock_password *pw) {
	if (pw->len != 0) {
...
static void append_ch(struct aetherlock_password *pw, uint32_t codepoint) {
...
static void schedule_input_idle(struct aetherlock_state *state) {
...
static void cancel_input_idle(struct aetherlock_state *state) {
	if (state->input_idle_timer) {
		loop_remove_timer(state->eventloop, state->input_idle_timer);
		state->input_idle_timer = NULL;
	}
...
static void schedule_password_clear(struct aetherlock_state *state) {
...
static void cancel_password_clear(struct aetherlock_state *state) {
	if (state->clear_password_timer) {
		loop_remove_timer(state->eventloop, state->clear_password_timer);
		state->clear_password_timer = NULL;
	}
...
static void submit_password(struct aetherlock_state *state) {
...
static void update_highlight(struct aetherlock_state *state) {
