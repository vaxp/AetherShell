...
enum background_mode parse_background_mode(const char *mode) {
	...
	aetherlock_log(LOG_ERROR, "Unsupported background mode: %s", mode);
...
cairo_surface_t *load_background_image(const char *path) {
	...
	if (!pixbuf) {
		aetherlock_log(LOG_ERROR, "Failed to load background image (%s).",
				err->message);
		return NULL;
	}
	...
	if (!image) {
		aetherlock_log(LOG_ERROR, "Failed to read background image.");
		return NULL;
	}
	if (cairo_surface_status(image) != CAIRO_STATUS_SUCCESS) {
		aetherlock_log(LOG_ERROR, "Failed to read background image: %s."
#if !HAVE_GDK_PIXBUF
				"\aetherlock was compiled without gdk_pixbuf support, so only"
