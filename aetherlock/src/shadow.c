...
void initialize_pw_backend(int argc, char **argv) {
	...
	if (!pwent) {
		aetherlock_log_errno(LOG_ERROR, "failed to getpwuid");
		exit(EXIT_FAILURE);
	}
	...
	if (strcmp(encpw, "x") == 0) {
		...
		if (!swent) {
			aetherlock_log_errno(LOG_ERROR, "failed to getspnam");
			exit(EXIT_FAILURE);
		}
		encpw = swent->sp_pwdp;
	}
	...
	if (setgid(getgid()) != 0) {
		aetherlock_log_errno(LOG_ERROR, "Unable to drop root");
		exit(EXIT_FAILURE);
	}
	if (setuid(getuid()) != 0) {
		aetherlock_log_errno(LOG_ERROR, "Unable to drop root");
	...
	if (setuid(0) != -1 || setgid(0) != -1) {
		aetherlock_log_errno(LOG_ERROR, "Unable to drop root (we shouldn't be "
			"able to restore it after setuid/setgid)");
		exit(EXIT_FAILURE);
	}

	/* This code does not run as root */
	aetherlock_log(LOG_DEBUG, "Prepared to authorize user %s", pwent->pw_name);
...
void run_pw_backend_child(void) {
	...
	while (1) {
		...
		if (c == NULL) {
			aetherlock_log_errno(LOG_ERROR, "crypt failed");
			exit(EXIT_FAILURE);
		}
