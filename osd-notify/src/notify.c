...
static guint32 create_new_notification(const char *app_name, guint32 replaces_id, const char *summary, const char *body, const char *icon, GVariant *actions, GVariant *hints, gint timeout, const char *desktop_entry, gint value) {
    ...
    if (do_not_disturb) {
        ...
        add_to_history(new_id, app_name, icon, summary, body, desktop_entry, value);
        emit_history_updated_signal(NULL);
        return new_id;
    }
    ...
    // إضافة الإشعار للسجل
    add_to_history(n->id, app_name, icon, summary, body, desktop_entry, value);
...
static void handle_method_call(GDBusConnection *connection, const gchar *sender,
    ...
    // طلب السجل (للكونترول سنتر)
        ...
        GVariantBuilder *builder = g_variant_builder_new(G_VARIANT_TYPE("a(usssssi)"));
        
        for (GList *l = history_list; l != NULL; l = l->next) {
            NotificationItem *item = (NotificationItem *)l->data;
            g_variant_builder_add(builder, "(usssssi)", 
                                  item->id, 
        ...
        g_dbus_method_invocation_return_value(invocation, g_variant_new("(a(usssssi))", builder));
