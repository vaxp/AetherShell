/*
 * css_provider.c — AetherShell Panel CSS Provider
 *
 * Loads the base stylesheet shipped with the panel, then layers a user-
...
static void ensure_user_css(const char *path)
{
    ...
    if (!g_file_test(path, G_FILE_TEST_EXISTS)) {
        const char *stub =
            "/* AetherShell Panel — user stylesheet\n"
            " *\n"
            " * Override any panel styles here.  Changes are applied live;\n"
