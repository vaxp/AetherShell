...
{
    /* 1. User config */
    char *p = g_build_filename(g_get_user_config_dir(), "aether", "AetherCore.json", NULL);
    ...
    g_free(exe);
    p = g_build_filename(dir, "..", "config", "AetherCore.json", NULL);
    g_free(dir);
    if (g_file_test(p, G_FILE_TEST_EXISTS)) return p;
    g_free(p);

    /* 3. Generate path (will be created on first save) */
    return g_build_filename(g_get_user_config_dir(), "aether", "AetherCore.json", NULL);
}

static char *user_css_path(void)
{
    return g_build_filename(g_get_user_config_dir(), "aether", "AetherCore-user.css", NULL);
}

static char *state_path(void)
{
    return g_build_filename(g_get_user_config_dir(), "aether",
                            "AetherCore-designer-state.json", NULL);
}
...
{
    /* Find the AetherCore binary next to the designer */
    char *exe = g_file_read_link("/proc/self/exe", NULL);
    char *dir = exe ? g_path_get_dirname(exe) : g_strdup(".");
    g_free(exe);
    char *AetherCore = g_build_filename(dir, "..", "AetherCore", NULL);
    g_free(dir);

    char *cmd = g_strdup_printf(
        "pkill -x AetherCore 2>/dev/null; sleep 0.4; '%s' &", AetherCore);
    g_free(AetherCore);
    system(cmd);
    g_free(cmd);
}
