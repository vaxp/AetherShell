/*
 * designer/main.c — AetherShell AetherCore Designer
...
/* AetherCore-plugin-api.h lives one level up */
#include "../AetherCore-plugin-api.h"

#define TITLE   "AetherShell AetherCore Designer"
#define W       1200
#define H       730
...
static const BuiltinDesc BUILTINS[] = {
  { "aether-appmenu",   "App Menu",       "top-left",   "🖥️"  },
  { "aether-clipboard", "Clipboard",      "top-left",   "📋"  },
  { "aether-workspaces","Workspaces",     "top-left",   "⬛"  },
  { "aether-clock",     "Clock",          "top-center", "🕐"  },
  { "aether-sni-tray",  "System Tray",    "top-right",  "📊"  },
  { "aether-keyboard",  "Keyboard",       "top-right",  "⌨️"  },
  { "aether-wifi",      "Wi-Fi",          "top-right",  "📶"  },
  { "aether-bt",        "Bluetooth",      "top-right",  "🔵"  },
  { "aether-mic",       "Microphone",     "top-right",  "🎙️"  },
  { "aether-volume",    "Volume",         "top-right",  "🔊"  },
  { "aether-battery",   "Battery",        "top-right",  "🔋"  },
  { "aether-search",    "Search",         "top-right",  "🔍"  },
  { "aether-notifs",    "Notifications",  "top-right",  "🔔"  },
  { "aether-cc",        "Control Center", "top-right",  "⚙️"  },
};
...
{
    switch (z) {
        case AETHER_PLUGIN_ZONE_LEFT:   return "top-left";
        case AETHER_PLUGIN_ZONE_CENTER: return "top-center";
        case AETHER_PLUGIN_ZONE_RIGHT:  return "top-right";
                               ...
                               int        *count)  /* running item count */
{
    ...
    while ((name = g_dir_read_name(dir)) != NULL) {
        ...
        /* Try to load the .so and call aether_AetherCore_plugin_init_v3 */
        ...
        dlerror();
        AetherAetherCorePluginInitFnV3 fn =
        ...
        AetherAetherCorePluginAPIv3 *api = fn();
        if (!api || api->api_version != AETHER_AetherCore_PLUGIN_API_VERSION) {
            dlclose(handle);
            g_free(id); g_free(full);
            continue;
        }
                       ...
                       gpointer                  _)
{
    ...
    } else if (g_strcmp0(action, "restart_AetherCore") == 0) {
        config_io_restart_AetherCore();
    }
...
int main(int argc, char *argv[])
{
    ...
    webkit_user_content_manager_register_script_message_handler(ucm, "AetherCoreDesigner");
    g_signal_connect(ucm, "script-message-received::AetherCoreDesigner",
