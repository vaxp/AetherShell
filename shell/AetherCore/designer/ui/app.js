/* =====================================================================
 * AetherShell AetherCore Designer — app.js
...
/* ── State ─────────────────────────────────────────────────────────── */
const state = {
  layout: null,   // AetherCore.json content (parsed)
...
function defaultStyle(pillId) {
  ...
  return {
    ...
    spacing: 0,   // px between elements inside the pill (0 = default)
    width: 0,   // min-width in px (0 = auto)
    height: 0,   // fixed height in px (0 = auto)
    borderOn: false,
    borderColor: '#ffffff',
    borderAlpha: 15,
  };
...
function sendToC(obj) {
  if (window.webkit?.messageHandlers?.AetherCoreDesigner) {
    window.webkit.messageHandlers.AetherCoreDesigner.postMessage(JSON.stringify(obj));
  } else {
...
window._aether = {
  init(layout, savedState, plugins) {
    ...
    if (savedState) {
      ...
      state.AetherCoreBg = savedState.AetherCoreBg || state.AetherCoreBg;
...
function defaultLayout() {
  return {
    AetherCore: { spacing: 0, margin: { top: 0, bottom: 0, left: 0, right: 0 } },
    layout: {
      'top-left': [{ type: 'pill', id: 'menu-pill', spacing: 0, plugins: ['aether-appmenu', 'aether-clipboard'] },
      { type: 'pill', id: 'workspaces-pill', spacing: 0, plugins: ['aether-workspaces'] }],
      'top-center': [{ type: 'pill', id: 'center-pill', spacing: 0, plugins: ['aether-clock'] }],
      'top-right': [{ type: 'pill', id: 'sys-pill', spacing: 0, plugins: ['aether-battery', 'aether-search', 'aether-notifs', 'aether-cc'] },
...
function autoSave() {
  ...
  sendToC({
    ...
    css: fullCSS,
    state: JSON.stringify({ styles: state.styles, AetherCoreBg: state.AetherCoreBg, winStyles: state.winStyles }),
  });
}
...
/* ── Zone rendering ────────────────────────────────────────────────── */
const ALL_ZONES = ['top-left', 'top-center', 'top-right', 'left', 'center', 'right', 'bottom-left', 'bottom-center', 'bottom-right'];
const VERTICAL_ZONES = new Set(['left', 'right']);

function renderZones() {
  /* Ensure all zone arrays exist */
  ALL_ZONES.forEach(zone => {
    if (!state.layout.layout[zone]) state.layout.layout[zone] = [];
  });
  ...
  ALL_ZONES.forEach(zone => {
    ...
    if (!el) return;
    el.innerHTML = '';
    /* Mark vertical zones so CSS can re-orient pills */
    el.dataset.orientation = VERTICAL_ZONES.has(zone) ? 'vertical' : 'horizontal';
...
function buildPillCard(pill, zone) {
  const isVertical = VERTICAL_ZONES.has(zone);
  const card = document.createElement('div');
  card.className = 'pill-card' +
    (state.selectedPill === pill.id ? ' selected' : '') +
    (isVertical ? ' pill-card-vertical' : '');
...
function selectPill(pillId) {
  ...
  /* Sync layout-level props (spacing/width/height) into style so inspector
   * always reflects the actual values stored in the JSON. */
  const found = findPill(pillId);
  if (found) {
    const s = state.styles[pillId];
    if (s.spacing === undefined) s.spacing = found.pill.spacing ?? 0;
    if (s.width === undefined) s.width = found.pill.width ?? 0;
    if (s.height === undefined) s.height = found.pill.height ?? 0;
    /* If already defined in style, prefer layout (source of truth) */
    s.spacing = found.pill.spacing ?? s.spacing;
    s.width = found.pill.width ?? s.width;
    s.height = found.pill.height ?? s.height;
  }
...
function showInspector(pillId) {
  ...
  document.getElementById('inspector-AetherCore').classList.remove('hidden');
  ...
  /* Container geometry */
  setRangeAuto('prop-spacing', 'prop-spacing-val', s.spacing ?? 0, 'px');
  setRangeAuto('prop-width', 'prop-width-val', s.width ?? 0, 'px');
  setRangeAuto('prop-height', 'prop-height-val', s.height ?? 0, 'px');
...
/* Like setRange but shows 'auto' when value is 0 */
function setRangeAuto(id, valId, num, unit) {
  const e = document.getElementById(id);
  const v = document.getElementById(valId);
  if (e) e.value = num;
  if (v) v.textContent = num === 0 ? 'auto' : num + unit;
}

function hideInspector() {
  document.getElementById('inspector-empty').classList.remove('hidden');
  document.getElementById('inspector-AetherCore').classList.add('hidden');
}

/* Wire inspector controls */
function wireInspector() {
  const controls = [
    ['prop-bg-alpha', 'prop-bg-alpha-val', '%', 'bgAlpha', false],
    ['prop-radius', 'prop-radius-val', 'px', 'radius', false],
    ['prop-margin-v', 'prop-margin-v-val', 'px', 'marginV', false],
    ['prop-margin-h', 'prop-margin-h-val', 'px', 'marginH', false],
    ['prop-padding', 'prop-padding-val', 'px', 'paddingH', false],
    ['prop-border-alpha', 'prop-border-alpha-val', '%', 'borderAlpha', false],
    /* Container geometry — show 'auto' when 0 */
    ['prop-spacing', 'prop-spacing-val', 'px', 'spacing', true],
    ['prop-width', 'prop-width-val', 'px', 'width', true],
    ['prop-height', 'prop-height-val', 'px', 'height', true],
  ];

  for (const [id, valId, unit, prop, autoZero] of controls) {
    document.getElementById(id).addEventListener('input', function () {
      const v = Number(this.value);
      document.getElementById(valId).textContent =
        (autoZero && v === 0) ? 'auto' : v + unit;
      if (state.selectedPill) {
        state.styles[state.selectedPill][prop] = v;
        /* spacing and size are layout properties — push into state.layout */
        if (prop === 'spacing' || prop === 'width' || prop === 'height') {
          const found = findPill(state.selectedPill);
          if (found) found.pill[prop] = v;
        }
        onStyleChange();
      }
    });
  ...
  /* AetherCore background */
  document.getElementById('prop-AetherCore-color').addEventListener('input', function () {
    state.AetherCoreBg.color = this.value;
    onStyleChange();
  });
  document.getElementById('prop-AetherCore-alpha').addEventListener('input', function () {
    document.getElementById('prop-AetherCore-alpha-val').textContent = this.value + '%';
    state.AetherCoreBg.alpha = Number(this.value);
    onStyleChange();
  });
...
function syncAetherCoreBgControls() {
  document.getElementById('prop-AetherCore-color').value = state.AetherCoreBg.color;
  document.getElementById('prop-AetherCore-alpha').value = state.AetherCoreBg.alpha;
  document.getElementById('prop-AetherCore-alpha-val').textContent = state.AetherCoreBg.alpha + '%';
}
...
function generateCSS() {
  ...
  for (const [pillId, s] of Object.entries(state.styles)) {
    ...
    /* Width/height: set BOTH min and max to pin the exact size.
     * This allows reducing below the natural size (max-width/height)
     * as well as enlarging (min-width/height). 0 = no constraint. */
    const w = s.width ?? 0;
    const h = s.height ?? 0;
    const sizeLines = [
      w > 0 ? `\n    min-width: ${w}px;\n    max-width: ${w}px;` : '',
...
function renderPreview() {
  ALL_ZONES.forEach(zone => {
    ...
    if (!el) return;
    el.innerHTML = '';
    /* Mark orientation so CSS can flex-direction correctly */
    el.dataset.orientation = VERTICAL_ZONES.has(zone) ? 'vertical' : 'horizontal';
    const pills = state.layout.layout[zone] || [];
    for (const pill of pills) {
      el.appendChild(buildPreviewPill(pill, zone));
    }
  });
...
function buildPreviewPill(pill, zone) {
  const isVertical = VERTICAL_ZONES.has(zone);
  const s = state.styles[pill.id] || defaultStyle(pill.id);
  const div = document.createElement('div');
  div.className = 'preview-pill' +
    (state.selectedPill === pill.id ? ' selected' : '') +
    (isVertical ? ' preview-pill-vertical' : '');
  ...
  div.style.cssText = `
    ...
    padding: ${isVertical ? `${Math.max(s.paddingH, 4)}px 0` : `0 ${Math.max(s.paddingH, 4)}px`};
...
function findPill(pillId) {
  for (const zone of ALL_ZONES) {
    if (!state.layout.layout[zone]) continue;
...
document.getElementById('btn-apply').addEventListener('click', () => {
  ...
  toast('Style applied — AetherCore updated live ✓', 'success');
});
...
document.getElementById('btn-restart').addEventListener('click', () => {
  ...
  setTimeout(() => sendToC({ action: 'restart_AetherCore' }), 400);
  toast('Saving and restarting AetherCore…');
});
...
function generateWindowCSS() {
  const lines = ['/* ── Window theme overrides (AetherCore Designer) ── */'];
