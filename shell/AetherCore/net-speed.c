 ...
 * Updated to AetherAetherCorePluginAPIv3
 *
 * Displays animated RX/TX chart + live speed labels in the AetherCore bar.
 */
...
#include <stdlib.h>
#include <string.h>
#include "AetherCore-plugin-api.h"   /* ← AetherAetherCorePluginAPIv3 */
...
/* ── Plugin visuals (AetherCore-bar slot appearance) ───────────────────────── */
...
/* create_widget — called by the AetherCore (AetherAetherCoreContext provided) */
static GtkWidget *create_widget(AetherAetherCoreContext *ctx)
{
...
static void destroy_widget(GtkWidget *widget)
{
     ...
     * This hook exists so the AetherCore can call it explicitly if needed. */
...
/*
 * on_config_changed — react to live config updates from AetherCore.conf.
