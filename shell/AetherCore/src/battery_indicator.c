...
static gboolean on_draw_battery(GtkWidget *widget, cairo_t *cr, gpointer data) {
    ...
    GtkOrientation orientation = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(widget), "battery-orientation"));
    
    double bw = (orientation == GTK_ORIENTATION_VERTICAL) ? 10.0 : 22.0;
    double bh = (orientation == GTK_ORIENTATION_VERTICAL) ? 22.0 : 10.0;
    ...
    double inner_max_h = bh - (gap * 2) - 1.0;
    ...
    if (orientation == GTK_ORIENTATION_VERTICAL) {
        double fill_h = inner_max_h * (current_percentage / 100.0);
        if (fill_h < 1.0 && current_percentage > 0.0) fill_h = 1.0;
        cairo_rectangle(cr, inner_x, inner_y + inner_max_h - fill_h, inner_max_w, fill_h);
    } else {
        double fill_w = inner_max_w * (current_percentage / 100.0);
        if (fill_w < 1.0 && current_percentage > 0.0) fill_w = 1.0;
        cairo_rectangle(cr, inner_x, inner_y, fill_w, inner_max_h);
    }
    cairo_fill(cr);
    ...
    if (charging) {
        ...
        if (orientation == GTK_ORIENTATION_VERTICAL) {
            cairo_move_to(cr, inner_x + 1, inner_y + inner_max_h / 2 - 1);
            cairo_line_to(cr, inner_x + inner_max_w / 2, inner_y + inner_max_h / 2 - 2);
            cairo_line_to(cr, inner_x + inner_max_w / 2, inner_y + inner_max_h / 2);
            cairo_line_to(cr, inner_x + inner_max_w - 1, inner_y + inner_max_h / 2 - 1);
        } else {
            cairo_move_to(cr, inner_x + inner_max_w / 2 - 1, inner_y + 1);
            cairo_line_to(cr, inner_x + inner_max_w / 2 - 2, inner_y + inner_max_h / 2);
            cairo_line_to(cr, inner_x + inner_max_w / 2, inner_y + inner_max_h / 2);
            cairo_line_to(cr, inner_x + inner_max_w / 2 - 1, inner_y + inner_max_h - 1);
        }
        cairo_stroke(cr);
    }
...
static void create_battery_popup_window(void) {
    ...
    AetherCore_window_backend_init_popup(GTK_WINDOW(battery_popup),
                                    "vaxpwy-battery",
                                    GDK_WINDOW_TYPE_HINT_POPUP_MENU,
                                    GTK_LAYER_SHELL_KEYBOARD_MODE_ON_DEMAND);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(battery_popup), GTK_LAYER_SHELL_EDGE_TOP, TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(battery_popup), GTK_LAYER_SHELL_EDGE_RIGHT, TRUE);
    AetherCore_window_backend_set_margin(GTK_WINDOW(battery_popup), GTK_LAYER_SHELL_EDGE_TOP, 32);
    AetherCore_window_backend_set_margin(GTK_WINDOW(battery_popup), GTK_LAYER_SHELL_EDGE_RIGHT, 0);
...
static void on_battery_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (battery_popup_visible) {
        ...
        AetherCore_window_backend_align_popup(GTK_WINDOW(battery_popup), battery_button, 300);
...
GtkWidget *get_battery_widget(void) {
    ...
    g_object_set_data(G_OBJECT(box), "battery-drawing-area", battery_drawing_area);

    return box;
}

void battery_indicator_set_orientation(GtkWidget *widget, GtkOrientation orientation) {
    GtkWidget *da = g_object_get_data(G_OBJECT(widget), "battery-drawing-area");
    if (!da) return;
    
    GtkOrientation current = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(da), "battery-orientation"));
    if (current == orientation) return;
    
    g_object_set_data(G_OBJECT(da), "battery-orientation", GINT_TO_POINTER(orientation));
    
    if (orientation == GTK_ORIENTATION_VERTICAL) {
        gtk_widget_set_size_request(da, 16, 30);
        gtk_orientable_set_orientation(GTK_ORIENTABLE(widget), GTK_ORIENTATION_VERTICAL);
    } else {
        gtk_widget_set_size_request(da, 30, 16);
