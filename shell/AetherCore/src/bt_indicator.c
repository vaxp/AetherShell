 ...
 * bt_indicator.c
 *
 * Bluetooth status icon in the AetherCore status-box.
...
static void create_bt_popup(void)
{
    ...
    AetherCore_window_backend_init_popup(GTK_WINDOW(bt_popup),
                                    "vaxpwy-bt",
                                    GDK_WINDOW_TYPE_HINT_POPUP_MENU,
                                    GTK_LAYER_SHELL_KEYBOARD_MODE_ON_DEMAND);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(bt_popup), GTK_LAYER_SHELL_EDGE_TOP, TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(bt_popup), GTK_LAYER_SHELL_EDGE_RIGHT, TRUE);
    AetherCore_window_backend_set_margin(GTK_WINDOW(bt_popup), GTK_LAYER_SHELL_EDGE_TOP, 32);
    AetherCore_window_backend_set_margin(GTK_WINDOW(bt_popup), GTK_LAYER_SHELL_EDGE_RIGHT, 0);
...
static void on_bti_clicked(GtkButton *btn, gpointer user_data)
{
    ...
    if (bt_popup_visible) {
        ...
        AetherCore_window_backend_align_popup(GTK_WINDOW(bt_popup), bti_btn, 300);
