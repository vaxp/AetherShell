...
static AetherCoreWorkspaceStateCallback s_workspace_cb = NULL;
static gpointer s_workspace_cb_data = NULL;
static AetherCoreKeyboardStateCallback s_keyboard_cb = NULL;
static gpointer s_keyboard_cb_data = NULL;

static AetherCoreWorkspaceState s_workspace_state = {-1, 0, 0, 0, 0};
static AetherCoreKeyboardState s_keyboard_state = {"US", 0};
...
static gboolean connect_wayfire_keyboard_ipc(void) {
    ...
    if (!socket_path || !socket_path[0]) {
        if (!s_wayfire_socket_warned) {
            g_warning("[AetherCore] WAYFIRE_SOCKET not found; Wayfire IPC disabled");
...
static gboolean init_aether_backend(void) {
    ...
    if (!AetherCore_window_backend_is_wayland()) return FALSE;
