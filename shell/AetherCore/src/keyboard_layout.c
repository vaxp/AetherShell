/*
 * keyboard_layout.c — Keyboard layout indicator for the Aether AetherCore
 */
...
static void on_keyboard_state_changed(const AetherCoreKeyboardState *state, gpointer user_data) {
    gchar **layouts;
    const char *display;
...
static void on_widget_destroy(GtkWidget *widget, gpointer user_data) {
    ...
    if (!s_widgets && s_registered) {
        AetherCore_compositor_backend_set_keyboard_callback(NULL, NULL);
        s_registered = FALSE;
    }
    g_free(w);
}
...
GtkWidget *create_keyboard_layout_widget(void) {
    ...
    if (!s_registered) {
        AetherCore_compositor_backend_set_keyboard_callback(on_keyboard_state_changed, NULL);
        s_registered = TRUE;
    }
