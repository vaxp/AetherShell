 ...
 * mic_indicator.c
 *
 * AetherCore microphone indicator:
 *   - A small icon button in the AetherCore bar that shows the current mic level.
...
static void create_mic_mixer_window(void)
{
    ...
    AetherCore_window_backend_init_popup(GTK_WINDOW(mic_mixer_window),
                                    "vaxpwy-mic-mixer",
                                    GDK_WINDOW_TYPE_HINT_POPUP_MENU,
                                    GTK_LAYER_SHELL_KEYBOARD_MODE_ON_DEMAND);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(mic_mixer_window), GTK_LAYER_SHELL_EDGE_TOP, TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(mic_mixer_window), GTK_LAYER_SHELL_EDGE_RIGHT, TRUE);
    AetherCore_window_backend_set_margin(GTK_WINDOW(mic_mixer_window), GTK_LAYER_SHELL_EDGE_TOP, 32);
    AetherCore_window_backend_set_margin(GTK_WINDOW(mic_mixer_window), GTK_LAYER_SHELL_EDGE_RIGHT, 0);
...
static void on_mi_clicked(GtkButton *btn, gpointer user_data)
{
    ...
    if (mic_mixer_visible) {
        ...
        AetherCore_window_backend_align_popup(GTK_WINDOW(mic_mixer_window), mi_btn, 300);
