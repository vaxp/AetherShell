...
void notifications_ui_set_relative_to(GtkWidget *notifications_window, GtkWidget *relative_to) {
    ...
    AetherCore_window_backend_align_popup(GTK_WINDOW(notifications_window), relative_to, 360);
}

GtkWidget* init_notifications_ui(void) {
    const gint AetherCore_offset = 0;
    ...
    AetherCore_window_backend_init_popup(GTK_WINDOW(popover),
                                    "vaxpwy-notifications",
                                    GDK_WINDOW_TYPE_HINT_POPUP_MENU,
                                    GTK_LAYER_SHELL_KEYBOARD_MODE_NONE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(popover), GTK_LAYER_SHELL_EDGE_TOP, TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(popover), GTK_LAYER_SHELL_EDGE_RIGHT, TRUE);
    AetherCore_window_backend_set_margin(GTK_WINDOW(popover), GTK_LAYER_SHELL_EDGE_TOP, AetherCore_offset);
    AetherCore_window_backend_set_margin(GTK_WINDOW(popover), GTK_LAYER_SHELL_EDGE_RIGHT, 0);
    ensure_rgba_visual(popover);
    
    GtkWidget *AetherCore_surface = gtk_event_box_new();
    gtk_widget_set_name(AetherCore_surface, "main-box");
    gtk_event_box_set_visible_window(GTK_EVENT_BOX(AetherCore_surface), TRUE);
    ensure_rgba_visual(AetherCore_surface);
    g_signal_connect(AetherCore_surface, "draw", G_CALLBACK(draw_notifications_background), NULL);
    gtk_container_add(GTK_CONTAINER(popover), AetherCore_surface);
    ...
    gtk_container_add(GTK_CONTAINER(AetherCore_surface), main_box);
