...
static const char *SIDEBAR_CSS =
    ...
    /* ══ TOP ROW: calendar AetherCore (right) ══ */
    "#cal_AetherCore {"
    "  padding: 14px;"
    "}"
    "#cal_nav_lbl {"
...
static GtkWidget* create_sidebar_content(void) {
     ...
     *
     *  ┌─────────────────────────────────────────┐
     *  │  [clock AetherCore]  │  [calendar AetherCore]     │  ← top_row (GtkGrid 1×2)
     *  ├────────┬────────┬────────┬──────────────┤
    ...
    /* ── clock AetherCore (left cell) ── */
    GtkWidget *clock_AetherCore = gtk_box_new(GTK_ORIENTATION_VERTICAL, 4);
    gtk_widget_set_name(clock_AetherCore, "clock_AetherCore");
    gtk_widget_set_valign(clock_AetherCore, GTK_ALIGN_CENTER);
    gtk_widget_set_halign(clock_AetherCore, GTK_ALIGN_FILL);

    lbl_time = gtk_label_new("00:00");
    gtk_widget_set_name(lbl_time, "lbl_time");
    gtk_label_set_xalign(GTK_LABEL(lbl_time), 0.5);
    gtk_box_pack_start(GTK_BOX(clock_AetherCore), lbl_time, FALSE, FALSE, 0);

    lbl_date = gtk_label_new("");
    gtk_widget_set_name(lbl_date, "lbl_date");
    gtk_label_set_xalign(GTK_LABEL(lbl_date), 0.5);
    gtk_box_pack_start(GTK_BOX(clock_AetherCore), lbl_date, FALSE, FALSE, 0);

    gtk_grid_attach(GTK_GRID(top_row), clock_AetherCore, 0, 0, 1, 1);

    /* ── calendar AetherCore (right cell) ── */
    GtkWidget *cal_AetherCore = gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);
    gtk_widget_set_name(cal_AetherCore, "cal_AetherCore");
    ...
    gtk_box_pack_start(GTK_BOX(cal_AetherCore), nav_m, FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(cal_AetherCore), nav_y, FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(cal_AetherCore), cal_grid, FALSE, FALSE, 4);

    gtk_grid_attach(GTK_GRID(top_row), cal_AetherCore, 1, 0, 1, 1);
...
GtkWidget *init_sidebar_popup(void) {
    ...
    AetherCore_window_backend_init_popup(GTK_WINDOW(popup),
    ...
    AetherCore_window_backend_set_margin(GTK_WINDOW(popup), GTK_LAYER_SHELL_EDGE_TOP, 0);
    AetherCore_window_backend_set_margin(GTK_WINDOW(popup), GTK_LAYER_SHELL_EDGE_LEFT, 8);
...
void sidebar_popup_set_relative_to(GtkWidget *popup, GtkWidget *relative_to) {
    ...
    AetherCore_window_backend_align_popup(GTK_WINDOW(popup), relative_to, SIDEBAR_POPUP_WIDTH);
}
