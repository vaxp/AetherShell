 ...
 * volume_indicator.c
 *
 * AetherCore volume indicator:
 *   - A small icon button in the AetherCore bar that shows the current volume level
...
static GtkWidget *vi_btn       = NULL;   /* AetherCore button               */
...
static void create_mixer_window(void)
{
    ...
    AetherCore_window_backend_init_popup(GTK_WINDOW(mixer_window),
                                    "vaxpwy-volume-mixer",
                                    GDK_WINDOW_TYPE_HINT_POPUP_MENU,
                                    GTK_LAYER_SHELL_KEYBOARD_MODE_ON_DEMAND);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(mixer_window), GTK_LAYER_SHELL_EDGE_TOP, TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(mixer_window), GTK_LAYER_SHELL_EDGE_RIGHT, TRUE);
    AetherCore_window_backend_set_margin(GTK_WINDOW(mixer_window), GTK_LAYER_SHELL_EDGE_TOP, 32);
    AetherCore_window_backend_set_margin(GTK_WINDOW(mixer_window), GTK_LAYER_SHELL_EDGE_RIGHT, 0);
...
static void on_vi_clicked(GtkButton *btn, gpointer user_data)
{
    ...
    if (mixer_visible) {
        ...
        AetherCore_window_backend_align_popup(GTK_WINDOW(mixer_window), vi_btn, 300);
...
GtkWidget *create_volume_indicator_widget(void)
{
     ...
     *  exists by the time the AetherCore is shown.)                                  */
