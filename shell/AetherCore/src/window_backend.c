...
typedef enum {
    AetherCore_BACKEND_UNKNOWN = 0,
    AetherCore_BACKEND_X11,
    AetherCore_BACKEND_WAYLAND
} AetherCoreBackendType;

/* Forward declarations */
static WindowBackendState *get_backend_state(GtkWindow *window);

static AetherCoreBackendType detected_backend = AetherCore_BACKEND_UNKNOWN;

/* ─── X11 exclusive-zone emulation ───────────────────────────────────
 * On Wayland, the compositor automatically keeps layer-shell popup windows
 * below the AetherCore bar's exclusive zone.  On X11 we must replicate this
 * ourselves: track the AetherCore bar's allocated height and push every popup
 * window's top-y coordinate so it never overlaps the AetherCore.
 * ──────────────────────────────────────────────────────────────────── */
static gint s_x11_exclusive_zone = 0; /* AetherCore bar height on X11 */
static GtkWindow *s_x11_AetherCore_window = NULL; /* the AetherCore bar window */

/* ─── EWMH struts: tell the WM to reserve space for the AetherCore ─────────
 * Without _NET_WM_STRUT_PARTIAL the window manager does NOT know that
 * the AetherCore is a dock and happily maximises application windows under it.
...
static void x11_apply_struts(GtkWindow *window) {
    ...
    if (!window || AetherCore_window_backend_is_wayland()) return;
    ...
    /* Determine which edge the AetherCore is on and its thickness. */
...
static void on_x11_AetherCore_size_allocate(GtkWidget *widget,
                                        GtkAllocation *alloc,
                                        gpointer data) {
    (void)data;
    if (alloc->height > 1)
        s_x11_exclusive_zone = alloc->height;
    /* Re-apply struts every time the AetherCore changes size so the WM always
...
static WindowBackendState *get_backend_state(GtkWindow *window) {
    ...
    state = g_object_get_data(G_OBJECT(window), "AetherCore-window-backend-state");
    if (state) return state;

    state = g_new0(WindowBackendState, 1);
    g_object_set_data_full(G_OBJECT(window),
                           "AetherCore-window-backend-state",
                           state,
                           g_free);
    return state;
}
...
void AetherCore_window_backend_detect(void) {
    ...
    if (detected_backend != AetherCore_BACKEND_UNKNOWN) return;
    if (!display) {
        detected_backend = AetherCore_BACKEND_X11;
        return;
    }
...
gboolean AetherCore_window_backend_is_wayland(void) {
    AetherCore_window_backend_detect();
    return detected_backend == AetherCore_BACKEND_WAYLAND;
}
...
static void sync_x11_window_position(GtkWindow *window) {
    ...
    if (state->anchors[GTK_LAYER_SHELL_EDGE_LEFT] &&
        state->anchors[GTK_LAYER_SHELL_EDGE_RIGHT]) {
        gint AetherCore_width = geometry.width - state->margins[GTK_LAYER_SHELL_EDGE_LEFT] -
                           state->margins[GTK_LAYER_SHELL_EDGE_RIGHT];
        if (AetherCore_width > 0) {
            gtk_window_resize(window, AetherCore_width, MAX(height, 1));
            width = AetherCore_width;
        }
     ...
     * above the AetherCore bar.  This mirrors what the Wayland compositor does
     * automatically via the layer-shell exclusive-zone mechanism.
     *
     * The condition is: this is NOT the AetherCore bar itself (auto_exclusive_zone
     * = FALSE), it IS anchored to the TOP, and the computed y falls inside
     * the reserved AetherCore area.  Windows that already carry a large enough TOP
...
/* ─── AetherCore bar: realize handler (standard WM-managed DOCK window) ───── */

static void on_backend_window_realize(GtkWidget *widget, gpointer user_data) {
    (void)user_data;

    if (AetherCore_window_backend_is_wayland()) return;
    sync_x11_window_position(GTK_WINDOW(widget));
    /* Apply EWMH struts now that we have a real X11 window handle.
     * The allocation may still be 1×1 here; on_x11_AetherCore_size_allocate
...
/* Used for the AetherCore bar (DOCK). WM manages it normally. */
static void init_x11_window(GtkWindow *window) {
     ...
     * to reserve space and keep app windows from overlapping the AetherCore. */
    ...
    s_x11_AetherCore_window = window;
}
...
void AetherCore_window_backend_init_AetherCore(GtkWindow *window, const char *namespace_name) {
    ...
    if (AetherCore_window_backend_is_wayland()) {
        gtk_layer_init_for_window(window);
        gtk_layer_set_namespace(window, namespace_name);
        gtk_layer_set_layer(window, GTK_LAYER_SHELL_LAYER_TOP);
        gtk_layer_set_keyboard_mode(window, GTK_LAYER_SHELL_KEYBOARD_MODE_NONE);
    } else {
        /* AetherCore bar: standard DOCK window managed by the WM. */
...
void AetherCore_window_backend_auto_exclusive_zone_enable(GtkWindow *window) {
    ...
    if (AetherCore_window_backend_is_wayland()) {
        ...
        /* On X11: track the AetherCore bar's height for exclusive-zone emulation
         * AND update EWMH struts whenever the AetherCore size changes. */
        GtkAllocation alloc;
        gtk_widget_get_allocation(GTK_WIDGET(window), &alloc);
        if (alloc.height > 1)
            s_x11_exclusive_zone = alloc.height;
        /* size-allocate fires after every layout pass — perfect for keeping
         * struts in sync with the actual rendered AetherCore size. */
        g_signal_connect(GTK_WIDGET(window), "size-allocate",
                         G_CALLBACK(on_x11_AetherCore_size_allocate), NULL);
    }
}
...
void AetherCore_window_backend_align_popup(GtkWindow *popup, GtkWidget *relative_to, gint popup_width) {
    ...
    popup_y = monitor.y; /* The popups already have a fixed TOP margin inside their css/logic if needed, but we align them based on the AetherCore's edge. Actually, wait. We need to know if the AetherCore is at the top or bottom! */

    AetherCore_window_backend_set_anchor(GTK_WINDOW(popup), GTK_LAYER_SHELL_EDGE_TOP, TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(popup), GTK_LAYER_SHELL_EDGE_LEFT, TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(popup), GTK_LAYER_SHELL_EDGE_RIGHT, FALSE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(popup), GTK_LAYER_SHELL_EDGE_BOTTOM, FALSE);
    ...
    AetherCore_window_backend_set_margin(GTK_WINDOW(popup), GTK_LAYER_SHELL_EDGE_LEFT, popup_x - monitor.x);
}
