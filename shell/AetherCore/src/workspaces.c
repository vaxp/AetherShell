/*
 * workspaces.c — Workspace switcher widget for the Aether AetherCore
 */
...
static void cairo_pill(cairo_t *cr, double x, double y, double w, double h) {
    ...
    if (w >= h) {
        ...
        cairo_arc(cr, x + r, y + r, r, M_PI, 2.0 * M_PI);
        cairo_arc(cr, x + r, y + h - r, r, 0, M_PI);
    }
    cairo_close_path(cr);
}
...
static void rebuild_workspace_buttons(WorkspaceData *data) {
    ...
    for (int i = 0; i < data->workspace_count; i++) {
        ...
        if (data->orientation == GTK_ORIENTATION_VERTICAL) {
            gtk_widget_set_size_request(da, DOT_W, dot->is_active ? PILL_W : DOT_H);
        } else {
...
static gboolean on_dot_click(GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
    ...
    AetherCore_compositor_backend_set_workspace(clicked->ws_data->output_id, target_x, target_y);
    return GDK_EVENT_STOP;
}

static void on_workspace_state_changed(const AetherCoreWorkspaceState *state, gpointer user_data) {
...
static void on_widget_destroy(GtkWidget *widget, gpointer user_data) {
    ...
    if (s_workspace_data == data) {
        AetherCore_compositor_backend_set_workspace_callback(NULL, NULL);
...
GtkWidget *create_workspaces_widget(void) {
    ...
    rebuild_workspace_buttons(data);
    AetherCore_compositor_backend_set_workspace_callback(on_workspace_state_changed, data);
    g_signal_connect(event_box, "destroy", G_CALLBACK(on_widget_destroy), data);
    g_object_set_data(G_OBJECT(event_box), "ws-data", data);
    return event_box;
}

void workspaces_set_orientation(GtkWidget *widget, GtkOrientation orientation) {
    WorkspaceData *data = g_object_get_data(G_OBJECT(widget), "ws-data");
    if (!data || data->orientation == orientation) return;
    
    data->orientation = orientation;
    gtk_orientable_set_orientation(GTK_ORIENTABLE(data->box), orientation);

    /* Swap margins on the event_box wrapper */
    if (orientation == GTK_ORIENTATION_VERTICAL) {
        ...
        gtk_widget_set_margin_start(data->box, 0);
        gtk_widget_set_margin_end(data->box, 0);
        gtk_widget_set_margin_top(data->box, 8);
        gtk_widget_set_margin_bottom(data->box, 8);
    } else {
        ...
        gtk_widget_set_margin_start(data->box, 8);
        gtk_widget_set_margin_end(data->box, 8);
        gtk_widget_set_margin_top(data->box, 0);
        gtk_widget_set_margin_bottom(data->box, 0);
    }
