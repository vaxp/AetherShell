/*
 * battery_indicator.c — Battery indicator widget for venom-panel.
 *
 * Ported from exim-panel. Shows a compact battery icon on the panel bar;
 * clicking it opens a popup window with detailed battery information
 * (status, charge %, battery size, last charge time, battery health).
 *
 * Uses window-backend so the popup works on both X11 and Wayland.
 */
...
static GtkWidget  *battery_button           = NULL;
static GtkWidget  *battery_drawing_area     = NULL;
static GtkWidget  *battery_popup            = NULL;
static GtkWidget  *status_value_label       = NULL;
static GtkWidget  *charge_value_label       = NULL;
static GtkWidget  *size_value_label         = NULL;
static GtkWidget  *last_charge_value_label  = NULL;
static GtkWidget  *health_value_label       = NULL;
...
static void update_battery_details_ui(void) {
    ...
    if (status_value_label)
        gtk_label_set_text(GTK_LABEL(status_value_label),
                           battery_state_to_text(battery_info.state));
...
#define FETCH_DOUBLE(key, field) \
    value = g_variant_lookup_value(properties, key, G_VARIANT_TYPE_DOUBLE); \
    if (value) { battery_info.field = g_variant_get_double(value); g_variant_unref(value); }

#define FETCH_UINT32(key, field) \
    value = g_variant_lookup_value(properties, key, G_VARIANT_TYPE_UINT32); \
    if (value) { battery_info.field = g_variant_get_uint32(value); g_variant_unref(value); }

#define FETCH_UINT64(key, field) \
    value = g_variant_lookup_value(properties, key, G_VARIANT_TYPE_UINT64); \
    if (value) { battery_info.field = g_variant_get_uint64(value); g_variant_unref(value); }

    FETCH_DOUBLE("Percentage",        percentage)
    FETCH_UINT32("State",             state)
    FETCH_DOUBLE("Energy",            energy)
    FETCH_DOUBLE("EnergyFull",        energy_full)
    FETCH_DOUBLE("EnergyFullDesign",  energy_full_design)
    FETCH_DOUBLE("Capacity",          capacity)
    FETCH_UINT64("UpdateTime",        update_time)
...
static GtkWidget *create_info_row(const char *title, GtkWidget **value_label_out) {
...
static void create_battery_popup_window(void) {
    GtkWidget *outer, *header, *header_icon, *title, *separator, *content;
    GdkScreen *screen;
    GdkVisual *visual;
    ...
    /* ── Window backend: layer-shell on Wayland, override_redirect on X11 ── */
    ...
    if (panel_window_backend_is_wayland()) {
        panel_window_backend_anchor_popup_to_panel(
            GTK_WINDOW(battery_popup),
            GTK_LAYER_SHELL_EDGE_RIGHT, 0);
    } else {
        /* X11: position near panel after realize */
        g_signal_connect(battery_popup, "realize",
    ...
    title       = gtk_label_new("Battery Info");
    ...
    gtk_box_pack_start(GTK_BOX(content),
        create_info_row("Status",         &status_value_label),       FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(content),
        create_info_row("Charge",         &charge_value_label),       FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(content),
        create_info_row("Battery Size",   &size_value_label),         FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(content),
        create_info_row("Last Charge",    &last_charge_value_label),  FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(content),
        create_info_row("Battery Health", &health_value_label),       FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(outer), content, TRUE, TRUE, 0);

    g_signal_connect(battery_popup, "key-press-event",
                     G_CALLBACK(on_battery_popup_key_press), NULL);
...
static void on_proxy_ready(GObject *source_object, GAsyncResult *res, gpointer user_data) {
    ...
    if (error) {
        g_printerr("[Battery] Error creating UPower proxy: %s\n", error->message);
        g_error_free(error);
        return;
    }

    g_signal_connect(upower_proxy, "g-properties-changed",
...
GtkWidget *get_battery_widget(void) {
    ...
    gtk_widget_set_margin_start(battery_button, 4);
    ...
    g_signal_connect(battery_button, "clicked",
                     G_CALLBACK(on_battery_button_clicked), NULL);
