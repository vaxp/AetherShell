/*
 * compositor-backend.c — Abstraction layer for compositor IPC.
 *
 * Detects and initialises one of three backends at startup:
 *   1. Aether   — custom Wayland protocol (aether_ipc_manager_v1)
 *   2. Wayfire  — JSON IPC over a Unix socket (WAYFIRE_SOCKET)
 *   3. X11      — EWMH _NET_CURRENT_DESKTOP + XKB extension
 *
 * Only one backend is active at a time.  Callers register callbacks via
 * panel_compositor_backend_set_workspace_callback() and
 * panel_compositor_backend_set_keyboard_callback() and are notified
 * whenever state changes without knowing which backend is in use.
 */

#include "compositor-backend.h"
#include "aether-ipc-v1-client-protocol.h"
#include "workspaces-xorg.h"
#include "kb-indicator-xorg.h"
...
static void handle_wayfire_keyboard_state(JsonObject *state) {
    ...
    /* ── Format A: possible-layouts (array) + layout-index ── */
    if (json_object_has_member(state, "possible-layouts") &&
        ...
        if (!layouts) return;
        if (idx < 0 || idx >= (int)json_array_get_length(layouts)) idx = 0;
        ...
        if (joined->len == 0) { g_string_free(joined, TRUE); return; }
    ...
    /* ── Format B: layout (string) + optional layout-index ── */
    if (json_object_has_member(state, "layout")) {
        const char *layout = json_object_get_string_member(state, "layout");
        if (!layout || !layout[0]) return;

        if (json_object_has_member(state, "layout-index"))
            idx = (int)json_object_get_int_member(state, "layout-index");

        g_strlcpy(s_keyboard_state.layouts, layout, sizeof(s_keyboard_state.layouts));
        s_keyboard_state.layout_index = idx;
        s_have_keyboard_state = TRUE;
        emit_keyboard_state();
        return;
...
static gboolean wayfire_process_buffer(void) {
    ...
    while (s_wayfire_buffer->len >= sizeof(guint32)) {
        ...
        if (root && JSON_NODE_HOLDS_OBJECT(root)) {
            ...
            /* Skip responses that indicate an error */
            ...
            if (json_object_has_member(obj, "event")) {
                /* ── Asynchronous event from watch subscription ── */
                ...
                if (g_strcmp0(ev, "keyboard-modifier-state-changed") == 0 &&
                    ...
                    JsonObject *state = json_object_get_object_member(obj, "state");
                    if (state) handle_wayfire_keyboard_state(state);
                }
            } else {
                /* ── Synchronous reply to get-keyboard-state ──
                 * Wayfire replies with the state object directly
                 * (no "event" wrapper).  Try both the root object
                 * itself and a nested "keyboard-state" key. */
                if (json_object_has_member(obj, "possible-layouts")) {
                    handle_wayfire_keyboard_state(obj);
                } else if (json_object_has_member(obj, "keyboard-state")) {
                    JsonNode *ks_node = json_object_get_member(obj, "keyboard-state");
                    if (ks_node && JSON_NODE_HOLDS_OBJECT(ks_node)) {
                        handle_wayfire_keyboard_state(json_node_get_object(ks_node));
                    }
                }
                /* Workspace-list replies have no keyboard fields — silently skip */
            }
        }
        g_object_unref(parser);
    }
...
void panel_compositor_backend_init(void) {
    ...
    {
        gchar *wp = resolve_wayfire_socket_path();
        if (wp != NULL) {
            g_free(wp);
