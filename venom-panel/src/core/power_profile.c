...
static gulong m_props_handler_id = 0;
...
static void on_proxy_ready(GObject *source_object, GAsyncResult *res, gpointer user_data) {
    ...
    m_props_handler_id = g_signal_connect(m_proxy, "g-properties-changed", G_CALLBACK(on_properties_changed), NULL);
...
void power_profile_init(PowerProfileChangedCallback cb, gpointer user_data) {
    ...
    if (m_proxy) {
        ...
        if (v) {
            if (g_variant_is_of_type(v, G_VARIANT_TYPE_STRING)) {
                if (m_callback) m_callback(g_variant_get_string(v, NULL), m_callback_data);
            }
            g_variant_unref(v);
        }
        return;
...
void power_profile_cleanup(void)
{
    ...
    if (m_proxy && m_props_handler_id) {
        g_signal_handler_disconnect(m_proxy, m_props_handler_id);
        m_props_handler_id = 0;
    }
