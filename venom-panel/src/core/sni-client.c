...
#define SNI_HIDDEN_ITEM_REMOVE_DELAY_MS 1500
...
typedef struct tray_item
{
    ...
    guint hidden_remove_timeout_id;
} tray_item_t;
...
static void schedule_tray_remove_item(const gchar *service);
static void schedule_hidden_item_removal(tray_item_t *item);
...
static void on_tray_box_destroy(GtkWidget *widget, gpointer user_data)
{
    (void)widget;
    (void)user_data;
    tray_box = NULL;
...
static void refresh_tray_layout(void)
{
    if (!tray_box)
    {
        return;
    ...
    gtk_widget_queue_resize(tray_box);
    gtk_widget_queue_draw(tray_box);

    GtkWidget *toplevel = gtk_widget_get_toplevel(tray_box);
...
static gboolean remove_item_on_idle(gpointer data)
{
    gchar *service = data;
...
static void schedule_tray_remove_item(const gchar *service)
{
    ...
    g_idle_add(remove_item_on_idle, g_strdup(service));
}
...
static void update_item_icon(tray_item_t *item)
{
    ...
    if (!has_icon || item_is_passive(item))
    {
        gtk_widget_hide(item->button);
        schedule_hidden_item_removal(item);
    } else
    {
        if (item->hidden_remove_timeout_id)
        {
            g_source_remove(item->hidden_remove_timeout_id);
            item->hidden_remove_timeout_id = 0;
        }
...
static void tray_item_free(gpointer data)
{
    ...
    if (item->notify_handler_id && item->proxy)
    {
        g_signal_handler_disconnect(item->proxy, item->notify_handler_id);
    }
...
static gboolean remove_hidden_item_timeout(gpointer data)
{
    gchar *service = data;
...
static void schedule_hidden_item_removal(tray_item_t *item)
{
    if (!item || !item->service || item->hidden_remove_timeout_id)
    {
        return;
    }

    item->hidden_remove_timeout_id = g_timeout_add(
        SNI_HIDDEN_ITEM_REMOVE_DELAY_MS,
        remove_hidden_item_timeout,
...
static void on_item_name_owner_changed(GObject *object, GParamSpec *pspec, gpointer user_data)
{
    tray_item_t *item = user_data;
    ...
    name_owner = g_dbus_proxy_get_name_owner(item->proxy);
    if (!name_owner || !*name_owner)
    {
        schedule_tray_remove_item(item->service);
    }
}
...
static void tray_add_item(const gchar *service)
{
    ...
    item->notify_handler_id = g_signal_connect(item->proxy, "notify::g-name-owner",
        G_CALLBACK(on_item_name_owner_changed), item);

    if (!g_dbus_proxy_get_name_owner(item->proxy))
    {
        tray_item_free(item);
        return;
    }
...
static void tray_remove_item(const gchar *service)
{
    ...
    {
        for (GList *it = pending_item_queue->head; it; )
        {
            GList *next = it->next;
            if (g_strcmp0((const gchar*)it->data, service) == 0)
            {
                g_free(it->data);
                g_queue_delete_link(pending_item_queue, it);
            }
            it = next;
        }
    }

    if (g_hash_table_remove(tray_items, service))
    {
        refresh_tray_layout();
        return;
    }

    gchar *bus_name = parse_bus_name(service);
    gchar *object_path = parse_object_path(service);
    gchar *matched_key = NULL;
    ...
    {
        tray_item_t *item = value;
        if (!item)
        {
            continue;
        }

        if ((bus_name && g_strcmp0(item->bus_name, bus_name) == 0) ||
            (bus_name && object_path &&
             g_strcmp0(item->bus_name, bus_name) == 0 &&
             g_strcmp0(item->object_path, object_path) == 0))
        {
            matched_key = g_strdup((const gchar*)key);
            break;
        }
    }
    ...
    if (matched_key)
    {
        g_hash_table_remove(tray_items, matched_key);
        g_free(matched_key);
    }
    ...
    const gchar *signal_name, GVariant *parameters, gpointer user_data)
{
    ...
    {
        g_variant_get(parameters, "(&s)", &service);
        schedule_tray_remove_item(service);
    }
}
...
static void clear_pending_items(void)
{
    if (pending_item_source_id)
    {
        g_source_remove(pending_item_source_id);
        pending_item_source_id = 0;
    }

    if (pending_item_queue)
    {
        g_queue_free_full(pending_item_queue, g_free);
        pending_item_queue = NULL;
    }
}
...
GtkWidget* create_sni_tray_widget(void)
{
    ...
    g_signal_connect(tray_box, "destroy", G_CALLBACK(on_tray_box_destroy), NULL);
