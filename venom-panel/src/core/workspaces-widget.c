/*
 * workspaces-widget.c — Workspace switcher builtin for venom-panel.
 *
 * Delegates workspace state tracking to compositor_backend, which
 * automatically selects the correct IPC mechanism:
 *
 *   Wayland (Aether)  — custom wl_protocol events
 *   Wayland (Wayfire) — WAYFIRE_SOCKET JSON IPC
 *   X11               — EWMH _NET_CURRENT_DESKTOP polling
 *
 * The widget renders a row of pill-shaped dots (active) or circles
 * (inactive) and sends switch requests back via compositor_backend.
 */
...
typedef struct {
    GtkWidget *box;
    int current_workspace;  /* flat index: y * grid_width + x */
    int workspace_count;
    int grid_width;
    int grid_height;
    int output_id;
...
/* One global slot so we can cancel the callback on widget destroy. */
...
static gboolean on_dot_draw(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
    ...
    if (dot->is_active) {
        /* Active workspace: filled pill, accent colour */
...
static void rebuild_workspace_buttons(WorkspaceData *data) {
    ...
    for (i = 0; i < data->workspace_count; i++) {
        ...
        gtk_style_context_add_class(gtk_widget_get_style_context(ev),
                                    "workspace-dot-event-box");
        /* Free the dot context when the event box is destroyed */
...
static gboolean on_dot_click(GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
    ...
    if (clicked->index == clicked->ws_data->current_workspace)
...
GtkWidget *create_workspaces_widget(void) {
    ...
    /* Container with translucent pill background */
    ...
    /* Inner box that holds the dot widgets */
    ...
    /* Sensible defaults shown before first backend event arrives */
    data->output_id         = -1;
    data->grid_width        = 1;
    data->grid_height       = 1;
    ...
    /* Subscribe to workspace changes via the active compositor backend.
     * Works on X11 (EWMH poll), Wayfire IPC, and Aether Wayland protocol. */
